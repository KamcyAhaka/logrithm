...
const DUMMY_INSIGHTS = {
  ...
  activityScore: 82,
  tags: ["typescript", "high output", "multi-repo", "pr focused", "full-stack"] // add to dummy too
};
...
export const generateInsights = async (
  ...
  try {
    ...
    const prompt = `
      ...
      - Languages: ${activity.repositories.slice(0, 5).map((r: any) => r.primaryLanguage?.name).filter(Boolean).join(', ')}
    ...
    // Strip markdown code blocks if Gemini wraps anyway
    ...
    try {
      ...
      // Fallback: try extracting just the JSON object
    ...
    // Sanitise tags defensively in case Gemini ignores the rules
    ...
    insights.tags = insights.tags
      .map((t: string) => t.replace(/[^a-z0-9\s]/gi, '').toLowerCase().trim())
      .filter(Boolean)
      .slice(0, 7);
